...
public class ContainerRepair extends Container
{
	...
	public ContainerRepair(InventoryPlayer playerInventory, final World worldIn, final BlockPos blockPosIn, EntityPlayer player)
	{
		...
		this.addSlotToContainer(new Slot(this.outputSlot, 2, 134, 47)
		{
			...
			public ItemStack func_190901_a(EntityPlayer p_190901_1_, ItemStack p_190901_2_)
			{
				...
				ContainerRepair.this.inputSlots.setInventorySlotContents(0, ItemStack.EMPTY_ITEM_STACK);

				if (ContainerRepair.this.materialCost > 0)
				{
					ItemStack itemstack = ContainerRepair.this.inputSlots.getStackInSlot(1);

					if (!itemstack.isNotValid() && itemstack.getStackSize() > ContainerRepair.this.materialCost)
					{
						itemstack.decreaseStackSize(ContainerRepair.this.materialCost);
						ContainerRepair.this.inputSlots.setInventorySlotContents(1, itemstack);
					}
					else
					{
						ContainerRepair.this.inputSlots.setInventorySlotContents(1, ItemStack.EMPTY_ITEM_STACK);
					}
				}
				else
				{
	...
	public void updateRepairOutput()
	{
		...
		{
			this.outputSlot.setInventorySlotContents(0, ItemStack.EMPTY_ITEM_STACK);
			this.maximumCost = 0;
		}
		else
		{
			...
			j = j + itemstack.getRepairCost() + (itemstack2.isNotValid() ? 0 : itemstack2.getRepairCost());
			this.materialCost = 0;
			...
			if (!itemstack2.isNotValid())
			{
				...
				if (itemstack1.isItemStackDamageable() && itemstack1.getItem().getIsRepairable(itemstack, itemstack2))
				{
					...
					for (i3 = 0; l2 > 0 && i3 < itemstack2.getStackSize(); ++i3)
					{
			...
			if (!itemstack1.isNotValid())
			{
				...
				if (!itemstack2.isNotValid() && k2 < itemstack2.getRepairCost())
				{
