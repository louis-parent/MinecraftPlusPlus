...
public class WorldGenBigMushroom extends WorldGenerator
{
	...
	public boolean generate(World worldIn, Random rand, BlockPos position)
	{
		...
		if (block == null)
		{
			block = rand.nextBoolean() ? Blocks.getBlock(Blocks.BROWN_MUSHROOM_BLOCK) : Blocks.getBlock(Blocks.RED_MUSHROOM_BLOCK);
		}
		...
		if (position.getY() >= 1 && position.getY() + i + 1 < 256)
		{
			...
			else
			{
				...
				if (block1 != Blocks.getBlock(Blocks.DIRT) && block1 != Blocks.getBlock(Blocks.GRASS) && block1 != Blocks.getBlock(Blocks.MYCELIUM))
				{
					return false;
				}
				else
				{
					...
					if (block == Blocks.getBlock(Blocks.RED_MUSHROOM_BLOCK))
					{
					...
					for (int l2 = k2; l2 <= position.getY() + i; ++l2)
					{
						...
						if (block == Blocks.getBlock(Blocks.BROWN_MUSHROOM_BLOCK))
						{
							j3 = 3;
						}
						...
						for (int l1 = k3; l1 <= l3; ++l1)
						{
							for (int i2 = j1; i2 <= k1; ++i2)
							{
								...
								if (block == Blocks.getBlock(Blocks.BROWN_MUSHROOM_BLOCK) || l2 < position.getY() + i)
								{
