...
public class ChunkPrimer
{
	...
	public IBlockState getBlockState(int x, int y, int z)
	{
		...
		return iblockstate == null ? getDefaultState() : iblockstate;
	}
	...
	/**
	 * Counting down from the highest block in the sky, find the first non-air
	 * block for the given location (actually, looks like mostly checks x, z+1?
	 * And actually checks only the very top sky block of actual x, z)
	 */
	public int findGroundBlockIdx(int x, int z)
	{
		...
		for (int j = 255; j >= 0; --j)
		{
			...
			if (iblockstate != null && iblockstate != getDefaultState())
			{
				return j;
			}
		}
