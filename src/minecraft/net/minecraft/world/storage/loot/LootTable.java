...
public class LootTable
{
	...
	private void shuffleItems(List<ItemStack> stacks, int p_186463_2_, Random rand)
	{
		...
		while (iterator.hasNext())
		{
			...
			else if (itemstack.getStackSize() > 1)
			{
				list.add(itemstack);
				iterator.remove();
			}
		...
		while (p_186463_2_ > 0 && !list.isEmpty())
		{
			...
			int i = MathHelper.getInt(rand, 1, itemstack2.getStackSize() / 2);
			ItemStack itemstack1 = itemstack2.splitStack(i);

			if (itemstack2.getStackSize() > 1 && rand.nextBoolean())
			{
				list.add(itemstack2);
			}
			else
			...
			if (itemstack1.getStackSize() > 1 && rand.nextBoolean())
			{
				list.add(itemstack1);
			}
			else
	...
	private List<Integer> getEmptySlotsRandomized(IInventory inventory, Random rand)
	{
		...
		{
			if (inventory.getStackInSlot(i).isNotValid())
			{
