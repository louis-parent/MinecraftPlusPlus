...
public class NetHandlerPlayServer implements INetHandlerPlayServer, ITickable
{
			...
			else
			{
				...
				else
				{
					...
					for (int j = 0; j < this.playerEntity.openContainer.inventorySlots.size(); ++j)
					{
						...
						ItemStack itemstack1 = itemstack.isNotValid() ? ItemStack.EMPTY_ITEM_STACK : itemstack;
		...
		if (this.playerEntity.openContainer.windowId == p_191985_1_.func_192613_a() && this.playerEntity.openContainer.getCanCraft(this.playerEntity))
		{
			...
			if (!p_191985_1_.func_192610_d().isEmpty())
			{
				for (CPacketRecipePlacement.ItemMove cpacketrecipeplacement$itemmove : p_191985_1_.func_192610_d())
				{
					...
					{
						int i = cpacketrecipeplacement$itemmove.field_192673_a.getStackSize();
						...
						else
						{
							...
							{
								itemstack1.increaseStackSize(i);
							}
						}

						if (itemstack.getStackSize() == i)
						{
							this.playerEntity.openContainer.putStackInSlot(cpacketrecipeplacement$itemmove.field_192674_b, ItemStack.EMPTY_ITEM_STACK);
						}
						else
						{
							itemstack.decreaseStackSize(i);
						}
					}
				}
			}
			...
			if (!p_191985_1_.func_192615_c().isEmpty())
			{
				for (CPacketRecipePlacement.ItemMove cpacketrecipeplacement$itemmove1 : p_191985_1_.func_192615_c())
				{
					...
					{
						int j = cpacketrecipeplacement$itemmove1.field_192673_a.getStackSize();

						if (itemstack2.getStackSize() == j)
						{
							this.playerEntity.inventory.removeStackFromSlot(cpacketrecipeplacement$itemmove1.field_192675_c);
						}
						else
						{
							itemstack2.decreaseStackSize(j);
						}
	...
	private boolean func_193074_a(ItemStack p_193074_1_, ItemStack p_193074_2_)
	{
		...
		if (itemstack.getStackSize() < p_193074_1_.getStackSize())
		{
			return false;
		}
		else
		{
			itemstack.setStackSize(p_193074_1_.getStackSize());
	...
	public void processCreativeInventoryAction(CPacketCreativeInventoryAction packetIn)
	{
		...
		if (this.playerEntity.interactionManager.isCreative())
		{
			...
			if (!itemstack.isNotValid() && itemstack.hasTagCompound() && itemstack.getTagCompound().hasKey("BlockEntityTag", 10))
			{
			...
			boolean flag2 = itemstack.isNotValid() || itemstack.getMetadata() >= 0 && itemstack.getStackSize() <= 64 && !itemstack.isNotValid();

			if (flag1 && flag2)
			{
				if (itemstack.isNotValid())
				{
					this.playerEntity.inventoryContainer.putStackInSlot(packetIn.getSlotId(), ItemStack.EMPTY_ITEM_STACK);
				}
				else
				{
		...
		else if ("MC|BSign".equals(s))
		{
			...
			try
			{
				...
				if (itemstack3.getItem() == Items.WRITABLE_BOOK && itemstack4.getItem() == Items.WRITABLE_BOOK)
				{
					...
					this.playerEntity.setItemStackToSlot(EntityHandSlot.MAINHAND, itemstack2);
				}
			}
