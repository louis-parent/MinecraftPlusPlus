...
public class TileEntityHopper extends TileEntityLockableLoot implements IHopper, ITickable
{
    ...
    private boolean isFull()
    {
        for (ItemStack itemstack : this.inventory)
        {
            if (itemstack.isNotValid() || itemstack.getStackSize() != itemstack.getMaxStackSize())
            {
                return false;
            }
        }
    ...
    private boolean transferItemsOut()
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    if (!this.getStackInSlot(i).isNotValid())
                    {
    ...
    private boolean isInventoryFull(IInventory inventoryIn, EnumFacing side)
    {
        if (inventoryIn instanceof ISidedInventory)
        {
            ...
            for (int k : aint)
            {
                ...
                if (itemstack1.isNotValid() || itemstack1.getStackSize() != itemstack1.getMaxStackSize())
                {
                    return false;
                }
            }
    ...
    private static boolean isInventoryEmpty(IInventory inventoryIn, EnumFacing side)
    {
        if (inventoryIn instanceof ISidedInventory)
        {
            ...
            for (int i : aint)
            {
                if (!isidedinventory.getStackInSlot(i).isNotValid())
                {
                    return false;
                }
            }
        ...
        else
        {
            ...
            for (int k = 0; k < j; ++k)
            {
                if (!inventoryIn.getStackInSlot(k).isNotValid())
                {
                    return false;
                }
            }
    ...
    private static boolean pullItemFromSlot(IHopper hopper, IInventory inventoryIn, int index, EnumFacing direction)
    {
        ...
        if (!itemstack.isNotValid() && canExtractItemFromSlot(inventoryIn, itemstack, index, direction))
        {
    ...
    public static ItemStack putStackInInventoryAllSlots(IInventory inventoryIn, IInventory stack, ItemStack side, @Nullable EnumFacing p_174918_3_)
    {
        if (stack instanceof ISidedInventory && p_174918_3_ != null)
        {
            ...
            for (int k = 0; k < aint.length && !side.isNotValid(); ++k)
            {
        ...
        else
        {
            ...
            for (int j = 0; j < i && !side.isNotValid(); ++j)
            {
    ...
    private static ItemStack insertStack(IInventory inventoryIn, IInventory stack, ItemStack index, int side, EnumFacing p_174916_4_)
    {
        ...
        if (canInsertItemInSlot(stack, index, side, p_174916_4_))
        {
            ...
            if (itemstack.isNotValid())
            {
                ...
                index = ItemStack.EMPTY_ITEM_STACK;
                flag = true;
            }
            else if (canCombine(itemstack, index))
            {
                int i = index.getMaxStackSize() - itemstack.getStackSize();
                int j = Math.min(index.getStackSize(), i);
                index.decreaseStackSize(j);
    ...
    private static boolean canCombine(ItemStack stack1, ItemStack stack2)
    {
        ...
        else if (stack1.getStackSize() > stack1.getMaxStackSize())
        {
            return false;
        }
        else
