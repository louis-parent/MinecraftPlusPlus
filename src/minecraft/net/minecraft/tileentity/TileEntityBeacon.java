...
public class TileEntityBeacon extends TileEntityLockable implements ITickable, ISidedInventory
{
    ...
    {
        return index == 0 ? this.payment : ItemStack.EMPTY_ITEM_STACK;
    }
    ...
    {
        if (index == 0 && !this.payment.isNotValid())
        {
            if (count >= this.payment.getStackSize())
            {
                ItemStack itemstack = this.payment;
                this.payment = ItemStack.EMPTY_ITEM_STACK;
                return itemstack;
            }
            else
            {
