...
public abstract class EntityLivingBase extends Entity
{
	...
	/**
	 * The yaw rotation the entity will be interpolated to. Used for
	 * teleporting.
	 */
	protected double interpTargetYaw;

	/**
	 * The pitch rotation the entity will be interpolated to. Used for
	 * teleporting.
	 */
	...
	protected ItemStack activeItemStack = ItemStack.EMPTY_ITEM_STACK;
	...
	/**
	 * Removes the given potion effect from the active potion map and returns
	 * it. Does not call cleanup callbacks for the end of the potion effect.
	 */
	...
	public boolean attackEntityFrom(DamageSource source, float amount)
	{
		...
		else
		{
			...
			else
			{
				...
				if ((source == DamageSource.anvil || source == DamageSource.fallingBlock) && !this.getItemStackFromSlot(EntityArmorSlot.HEAD).isNotValid())
				{
					this.getItemStackFromSlot(EntityArmorSlot.HEAD).damageItem((int) (amount * 4.0F + this.rand.nextFloat() * amount * 2.0F), this);
					amount *= 0.75F;
				}
	...
	/**
	 * Determines whether the entity can block the damage source based on the
	 * damage source's location, whether the damage source is blockable, and
	...
	/**
	 * Constructs a knockback vector from the given direction ratio and
	 * magnitude and adds it to the entity's velocity. If it is on the ground
	 * (i.e. {@code this.onGround}), the Y-velocity is increased as well,
	 * clamping it to {@code
	 * .4}.
	 * 
	 * The entity's existing horizontal velocity is halved, and if the entity is
	 * on the ground the Y-velocity is too.
	 * 
	 * @param strength
	 *            Magnitude of the knockback vector, and also the Y-velocity to
	...
	/**
	 * Returns the maximum health of the entity (what it is able to regenerate
	 * up to, what it spawned with, etc)
	 */
	...
	{
		return this.getItemStackFromSlot(EntityHandSlot.MAINHAND);
	}

	public ItemStack getHeldItemOffhand()
	{
		return this.getItemStackFromSlot(EntityHandSlot.OFFHAND);
	}
	...
	public void setHeldItem(EnumHand hand, ItemStack stack)
	{
		if (hand == EnumHand.MAIN_HAND)
		{
			this.setItemStackToSlot(EntityHandSlot.MAINHAND, stack);
		}
		else
		{
			...
			this.setItemStackToSlot(EntityHandSlot.OFFHAND, stack);
		}
	}
	...
	public void onUpdate()
	{
		...
		if (!this.world.isRemote)
		{
			...
			for (EntityEquipmentSlot entityequipmentslot : EntityEquipmentSlot.values())
			{
				...
				if (entityequipmentslot instanceof EntityHandSlot)
				{
					itemstack = this.handInventory.get(entityequipmentslot.getIndex());
				}
				else if (entityequipmentslot instanceof EntityArmorSlot)
				{
				...
				if (!ItemStack.areItemStacksEqual(itemstack1, itemstack))
				{
					...
					if (entityequipmentslot instanceof EntityHandSlot)
					{
						this.handInventory.set(entityequipmentslot.getIndex(), itemstack1.isNotValid() ? ItemStack.EMPTY_ITEM_STACK : itemstack1.copy());
					}
					else if (entityequipmentslot instanceof EntityArmorSlot)
					{
						this.armorArray.set(entityequipmentslot.getIndex(), itemstack1.isNotValid() ? ItemStack.EMPTY_ITEM_STACK : itemstack1.copy());
					}
				}
			}
	...
	public void setActiveHand(EnumHand hand)
	{
		...
		if (!itemstack.isNotValid() && !this.isHandActive())
		{
	...
	public void notifyDataManagerChange(DataParameter<?> key)
	{
		...
		{
			if (this.isHandActive() && this.activeItemStack.isNotValid())
			{
				this.activeItemStack = this.getHeldItem(this.getActiveHand());

				if (!this.activeItemStack.isNotValid())
				{
					this.activeItemStackUseCount = this.activeItemStack.getMaxItemUseDuration();
				}
			}
			else if (!this.isHandActive() && !this.activeItemStack.isNotValid())
			{
	...
	{
		if (!stack.isNotValid() && this.isHandActive())
		{
	...
	/**
	 * Teleports the entity to the specified location. Used for Enderman and
