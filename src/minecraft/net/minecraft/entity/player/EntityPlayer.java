...
public abstract class EntityPlayer extends EntityLivingBase
{
	...
	/**
	 * The location of the bed the player is sleeping in, or {@code null} if
	...
	private ItemStack itemStackMainHand = ItemStack.EMPTY_ITEM_STACK;
	...
	public void onUpdate()
	{
		...
		if (!ItemStack.areItemStacksEqual(this.itemStackMainHand, itemstack))
		{
			...
			this.itemStackMainHand = itemstack.isNotValid() ? ItemStack.EMPTY_ITEM_STACK : itemstack.copy();
		}
	...
	protected void func_190776_cN()
	{
		for (int i = 0; i < this.inventory.getSizeInventory(); ++i)
		{
			...
			if (!itemstack.isNotValid() && EnchantmentHelper.func_190939_c(itemstack))
			{
	...
	{
		return this.dropItem(this.inventory.decrStackSize(this.inventory.currentItem, dropAll && !this.inventory.getCurrentItem().isNotValid() ? this.inventory.getCurrentItem().getStackSize() : 1), false, true);
	}
	...
	public EntityItem dropItem(ItemStack droppedItem, boolean dropAround, boolean traceItem)
	{
		...
		else
		{
			...
			if (traceItem)
			{
				...
				{
					this.addStat(StatList.getDroppedObjectStats(itemstack.getItem()), droppedItem.getStackSize());
				}
	...
	public EnumActionResult func_190775_a(Entity p_190775_1_, EnumHand p_190775_2_)
	{
		...
		else
		{
			...
			ItemStack itemstack1 = itemstack.isNotValid() ? ItemStack.EMPTY_ITEM_STACK : itemstack.copy();

			if (p_190775_1_.processInitialInteract(this, p_190775_2_))
			{
				if (this.capabilities.isCreativeMode && itemstack == this.getHeldItem(p_190775_2_) && itemstack.getStackSize() < itemstack1.getStackSize())
				{
			...
			{
				if (!itemstack.isNotValid() && p_190775_1_ instanceof EntityLivingBase)
				{
					...
					{
						if (itemstack.isNotValid() && !this.capabilities.isCreativeMode)
						{
							this.setHeldItem(p_190775_2_, ItemStack.EMPTY_ITEM_STACK);
						}
			...
			if (!targetEntity.hitByEntity(this))
			{
				...
				if (f > 0.0F || f1 > 0.0F)
				{
					...
					if (flag5)
					{
						...
						if (!itemstack1.isNotValid() && entity instanceof EntityLivingBase)
						{
							itemstack1.hitEntity((EntityLivingBase) entity, this);

							if (itemstack1.isNotValid())
							{
								this.setHeldItem(EnumHand.MAIN_HAND, ItemStack.EMPTY_ITEM_STACK);
							}
						}
	...
	public boolean canOpen(LockCode code)
	{
		...
		{
			ItemStack itemstack = this.getHeldItemMainhand();
			return !itemstack.isNotValid() && itemstack.hasDisplayName() ? itemstack.getDisplayName().equals(code.getLock()) : false;
		}
	}
