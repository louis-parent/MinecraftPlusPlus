...
public abstract class EntityLiving extends EntityLivingBase
{
	...
	protected int getExperiencePoints(EntityPlayer player)
	{
		if (this.experienceValue > 0)
		{
			...
			{
				if (!this.inventoryArmor.get(j).isNotValid() && this.inventoryArmorDropChances[j] <= 1.0F)
				{
			...
			{
				if (!this.inventoryHands.get(k).isNotValid() && this.inventoryHandsDropChances[k] <= 1.0F)
				{
					i += 1 + this.rand.nextInt(3);
				}
			}
	...
	public void spawnExplosionParticle()
	{
		if (this.world.isRemote)
		{
			for (int i = 0; i < 20; ++i)
			{
				...
				double d3 = 10.0D;
				this.world.spawnParticle(EnumParticleTypes.EXPLOSION_NORMAL, this.posX + this.rand.nextFloat() * this.width * 2.0F - this.width - d0 * 10.0D, this.posY + this.rand.nextFloat() * this.height - d1 * 10.0D, this.posZ + this.rand.nextFloat() * this.width * 2.0F - this.width - d2 * 10.0D, d0, d1, d2);
			}
		}
		else
		{
	...
	public void onLivingUpdate()
	{
		...
		if (!this.world.isRemote && this.canPickUpLoot() && !this.dead && this.world.getGameRules().getBoolean("mobGriefing"))
		{
			...
			{
				if (!entityitem.isDead && !entityitem.getEntityItem().isNotValid() && !entityitem.cannotPickup())
				{
	...
	protected void updateEquipmentIfNeeded(EntityItem itemEntity)
	{
		...
		if (flag && this.canEquipItem(itemstack))
		{
			...
			if (!itemstack1.isNotValid() && this.rand.nextFloat() - 0.1F < d0)
			{
			...
			this.onItemPickup(itemEntity, itemstack.getStackSize());
			itemEntity.setDead();
		}
	}
	...
	public ItemStack getItemStackFromSlot(EntityEquipmentSlot slot)
	{
		...
		{
			return this.inventoryHands.get(slot.getIndex());

		}
		else if (slot instanceof EntityArmorSlot)
		{
			return this.inventoryArmor.get(slot.getIndex());
	...
	public void setItemStackToSlot(EntityEquipmentSlot slot, ItemStack stack)
	{
		if (slot instanceof EntityHandSlot)
		{
			this.inventoryHands.set(slot.getIndex(), stack);
		}
		else if (slot instanceof EntityArmorSlot)
		{
			this.inventoryArmor.set(slot.getIndex(), stack);
		}
	}
	...
	protected void dropEquipment(boolean wasRecentlyHit, int lootingModifier)
	{
		for (EntityEquipmentSlot entityequipmentslot : EntityEquipmentSlot.values())
		{
			...
			if (!itemstack.isNotValid() && !EnchantmentHelper.func_190939_c(itemstack) && (wasRecentlyHit || flag) && this.rand.nextFloat() - lootingModifier * 0.01F < d0)
			{
	...
	protected void setEnchantmentBasedOnDifficulty(DifficultyInstance difficulty)
	{
		...
		if (!this.getHeldItemMainhand().isNotValid() && this.rand.nextFloat() < 0.25F * f)
		{
			this.setItemStackToSlot(EntityHandSlot.MAINHAND, EnchantmentHelper.addRandomEnchantment(this.rand, this.getHeldItemMainhand(), (int) (5.0F + f * this.rand.nextInt(18)), false));
		}
		...
		for (EntityEquipmentSlot entityequipmentslot : EntityEquipmentSlot.values())
		{
			if (entityequipmentslot instanceof EntityArmorSlot)
			{
				...
				if (!itemstack.isNotValid() && this.rand.nextFloat() < 0.5F * f)
				{
	...
	public boolean replaceItemInInventory(int inventorySlot, ItemStack itemStackIn)
	{
		...
		if (!itemStackIn.isNotValid() && !isItemStackInSlot(entityequipmentslot, itemStackIn) && entityequipmentslot != EntityArmorSlot.HEAD)
		{
			return false;
		}
		else
	...
	public static boolean isItemStackInSlot(EntityEquipmentSlot slotIn, ItemStack stack)
	{
		...
		return entityequipmentslot == slotIn || entityequipmentslot == EntityHandSlot.MAINHAND && slotIn == EntityHandSlot.OFFHAND || entityequipmentslot == EntityHandSlot.OFFHAND && slotIn == EntityHandSlot.MAINHAND;
	}
