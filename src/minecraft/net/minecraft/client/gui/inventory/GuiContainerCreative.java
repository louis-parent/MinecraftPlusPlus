...
public class GuiContainerCreative extends InventoryEffectRenderer
{
	...
	protected void handleMouseClick(@Nullable Slot slotIn, int slotId, int mouseButton, ClickType type)
	{
		...
		if (slotIn == null && selectedTabIndex != CreativeTabs.INVENTORY.getTabIndex() && type != ClickType.QUICK_CRAFT)
		{
			...
			if (!inventoryplayer1.getItemStack().isNotValid())
			{
				if (mouseButton == 0)
				{
					...
					inventoryplayer1.setItemStack(ItemStack.EMPTY_ITEM_STACK);
				}
		...
		else
		{
			...
			if (slotIn == this.destroyItemSlot && flag)
			{
				...
				{
					this.mc.playerController.sendSlotPacket(ItemStack.EMPTY_ITEM_STACK, j);
				}
			}
			else if (selectedTabIndex == CreativeTabs.INVENTORY.getTabIndex())
			{
				...
				{
					this.mc.player.inventory.setItemStack(ItemStack.EMPTY_ITEM_STACK);
				}
				...
				else if (type == ClickType.THROW && !this.mc.player.inventory.getItemStack().isNotValid())
				{
			...
			else if (type != ClickType.QUICK_CRAFT && slotIn.inventory == basicInventory)
			{
				...
				{
					if (!itemstack7.isNotValid() && mouseButton >= 0 && mouseButton < 9)
					{
						ItemStack itemstack10 = itemstack7.copy();
						itemstack10.setStackSize(itemstack10.getMaxStackSize());
				...
				{
					if (inventoryplayer.getItemStack().isNotValid() && slotIn.getHasStack())
					{
						ItemStack itemstack9 = slotIn.getStack().copy();
						itemstack9.setStackSize(itemstack9.getMaxStackSize());
				...
				{
					if (!itemstack7.isNotValid())
					{
						ItemStack itemstack8 = itemstack7.copy();
						itemstack8.setStackSize(mouseButton == 0 ? 1 : itemstack8.getMaxStackSize());
				...
				if (!itemstack5.isNotValid() && !itemstack7.isNotValid() && itemstack5.isItemEqual(itemstack7) && ItemStack.areItemStackTagsEqual(itemstack5, itemstack7))
				{
					if (mouseButton == 0)
					{
						if (flag)
						...
						else if (itemstack5.getStackSize() < itemstack5.getMaxStackSize())
						{
							itemstack5.increaseStackSize(1);
						}
					}
					else
					{
						itemstack5.decreaseStackSize(1);
					}
				}
				else if (!itemstack7.isNotValid() && itemstack5.isNotValid())
				{
				...
				{
					inventoryplayer.getItemStack().decreaseStackSize(1);
				}
			}
			else if (this.inventorySlots != null)
			{
				ItemStack itemstack3 = slotIn == null ? ItemStack.EMPTY_ITEM_STACK : this.inventorySlots.getSlot(slotIn.slotNumber).getStack();
				...
				else if (slotIn != null)
				{
					...
					else if (type == ClickType.THROW && !itemstack3.isNotValid())
					{
						ItemStack itemstack2 = itemstack3.copy();
						itemstack2.setStackSize(mouseButton == 0 ? 1 : itemstack2.getMaxStackSize());
			...
			for (int j = 0; j < 9; ++j)
			{
				...
				if (hotbarsnapshot.isEmpty())
				{
					for (int k = 0; k < 9; ++k)
					{
						...
						{
							guicontainercreative$containercreative.itemList.add(ItemStack.EMPTY_ITEM_STACK);
						}
					}
				}
				else
	...
	/**
	 * Draws the given tab and its background, deciding whether to highlight the
	 * tab or not based off of the selected index.
	 */
		...
		public void scrollTo(float p_148329_1_)
		{
			...
			for (int k = 0; k < 5; ++k)
			{
				for (int l = 0; l < 9; ++l)
				{
					...
					{
						GuiContainerCreative.basicInventory.setInventorySlotContents(l + k * 9, ItemStack.EMPTY_ITEM_STACK);
					}
				}
			}
		}
