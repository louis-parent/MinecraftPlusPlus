...
public abstract class GuiContainer extends GuiScreen
{
    ...
    /** Used when touchscreen is enabled */
    private ItemStack draggedStack = ItemStack.EMPTY_ITEM_STACK;
    ...
    /** Used when touchscreen is enabled */
    private ItemStack returningStack = ItemStack.EMPTY_ITEM_STACK;
    ...
    private ItemStack shiftClickedSlot = ItemStack.EMPTY_ITEM_STACK;
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
        ...
        ItemStack itemstack = this.draggedStack.isNotValid() ? inventoryplayer.getItemStack() : this.draggedStack;

        if (!itemstack.isNotValid())
        {
            int j2 = 8;
            int k2 = this.draggedStack.isNotValid() ? 8 : 16;
            String s = null;
            ...
            {
                itemstack = itemstack.copy();
                itemstack.setStackSize(this.dragSplittingRemnant);
        ...
        if (!this.returningStack.isNotValid())
        {
            ...
            {
                f = 1.0F;
                this.returningStack = ItemStack.EMPTY_ITEM_STACK;
            }
    ...
    {
        if (this.mc.player.inventory.getItemStack().isNotValid() && this.theSlot != null && this.theSlot.getHasStack())
        {
    ...
    private void drawItemStack(ItemStack stack, int x, int y, String altText)
    {
        ...
        this.itemRender.renderItemOverlayIntoGUI(this.fontRendererObj, stack, x, y - (this.draggedStack.isNotValid() ? 0 : 8), altText);
        this.zLevel = 0.0F;
    ...
    private void drawSlot(Slot slotIn)
    {
        ...
        boolean flag = false;
        boolean flag1 = slotIn == this.clickedSlot && !this.draggedStack.isNotValid() && !this.isRightMouseClick;
        ItemStack itemstack1 = this.mc.player.inventory.getItemStack();
        String s = null;

        if (slotIn == this.clickedSlot && !this.draggedStack.isNotValid() && this.isRightMouseClick && !itemstack.isNotValid())
        {
            itemstack = itemstack.copy();
            itemstack.setStackSize(itemstack.getStackSize() / 2);
        }
        else if (this.dragSplitting && this.dragSplittingSlots.contains(slotIn) && !itemstack1.isNotValid())
        {
            ...
            if (Container.canAddItemToSlot(slotIn, itemstack1, true) && this.inventorySlots.canDragIntoSlot(slotIn))
            {
                ...
                flag = true;
                Container.computeStackSize(this.dragSplittingSlots, this.dragSplittingLimit, itemstack, slotIn.getStack().isNotValid() ? 0 : slotIn.getStack().getStackSize());
        ...
        if (itemstack.isNotValid() && slotIn.canBeHovered())
        {
    ...
    private void updateDragSplitting()
    {
        ...
        if (!itemstack.isNotValid() && this.dragSplitting)
        {
            ...
            {
                this.dragSplittingRemnant = itemstack.getStackSize();

                for (Slot slot : this.dragSplittingSlots)
                {
                    ItemStack itemstack1 = itemstack.copy();
                    ItemStack itemstack2 = slot.getStack();
                    int i = itemstack2.isNotValid() ? 0 : itemstack2.getStackSize();
                    Container.computeStackSize(this.dragSplittingSlots, this.dragSplittingLimit, itemstack1, i);
                    int j = Math.min(itemstack1.getMaxStackSize(), slot.getItemStackLimit(itemstack1));

                    if (itemstack1.getStackSize() > j)
                    {
                        itemstack1.setStackSize(j);
                    }

                    this.dragSplittingRemnant -= itemstack1.getStackSize() - i;
                }
            }
        }
    }
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException
    {
        ...
        if (mouseButton == 0 || mouseButton == 1 || flag)
        {
            ...
            if (this.mc.gameSettings.touchscreen && flag1 && this.mc.player.inventory.getItemStack().isNotValid())
            {
            ...
            if (l != -1)
            {
                if (this.mc.gameSettings.touchscreen)
                {
                    if (slot != null && slot.getHasStack())
                    {
                        ...
                        this.draggedStack = ItemStack.EMPTY_ITEM_STACK;
                ...
                {
                    if (this.mc.player.inventory.getItemStack().isNotValid())
                    {
                        ...
                        else
                        {
                            ...
                            if (flag2)
                            {
                                this.shiftClickedSlot = slot != null && slot.getHasStack() ? slot.getStack().copy() : ItemStack.EMPTY_ITEM_STACK;
    ...
    protected void mouseClickMove(int mouseX, int mouseY, int clickedMouseButton, long timeSinceLastClick)
    {
        ...
        if (this.clickedSlot != null && this.mc.gameSettings.touchscreen)
        {
            ...
            {
                if (this.draggedStack.isNotValid())
                {
                    if (slot != this.clickedSlot && !this.clickedSlot.getStack().isNotValid())
                    {
                        this.draggedStack = this.clickedSlot.getStack().copy();
                    }
                }
                else if (this.draggedStack.getStackSize() > 1 && slot != null && Container.canAddItemToSlot(slot, this.draggedStack, false))
                {
                    long i = Minecraft.getSystemTime();
                    ...
                    if (this.currentDragTargetSlot == slot)
                    {
                        if (i - this.dragItemDropDelay > 500L)
                        {
                            ...
                            this.draggedStack.decreaseStackSize(1);
                        }
                    }
                    else
                    {
        ...
        else if (this.dragSplitting && slot != null && !itemstack.isNotValid() && (itemstack.getStackSize() > this.dragSplittingSlots.size() || this.dragSplittingLimit == 2) && Container.canAddItemToSlot(slot, itemstack, true) && slot.isItemValid(itemstack) && this.inventorySlots.canDragIntoSlot(slot))
        {
    ...
    protected void mouseReleased(int mouseX, int mouseY, int state)
    {
        ...
        if (this.doubleClick && slot != null && state == 0 && this.inventorySlots.canMergeSlot(ItemStack.EMPTY_ITEM_STACK, slot))
        {
            if (isShiftKeyDown())
            {
                if (!this.shiftClickedSlot.isNotValid())
                {
        ...
        else
        {
            ...
            if (this.clickedSlot != null && this.mc.gameSettings.touchscreen)
            {
                if (state == 0 || state == 1)
                {
                    if (this.draggedStack.isNotValid() && slot != this.clickedSlot)
                    {
                    ...
                    if (k != -1 && !this.draggedStack.isNotValid() && flag2)
                    {
                    ...
                    else if (!this.draggedStack.isNotValid())
                    {
                        this.touchUpX = mouseX - i;
                        this.touchUpY = mouseY - j;
                        this.returningStackDestSlot = this.clickedSlot;
            ...
            else if (!this.mc.player.inventory.getItemStack().isNotValid())
            {
    ...
    {
        if (this.mc.player.inventory.getItemStack().isNotValid() && this.theSlot != null)
        {
            for (int i = 0; i < 9; ++i)
            {
