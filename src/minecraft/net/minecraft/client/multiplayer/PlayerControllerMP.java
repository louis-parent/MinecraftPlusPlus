...
public class PlayerControllerMP
{
	...
	private ItemStack currentItemHittingBlock = ItemStack.EMPTY_ITEM_STACK;
	...
	public boolean onPlayerDestroyBlock(BlockPos pos)
	{
		...
		if (this.currentGameType.isCreative() && !this.mc.player.getHeldItemMainhand().isNotValid() && this.mc.player.getHeldItemMainhand().getItem() instanceof ItemSword)
		{
			return false;
		}
		else
			...
			else
			{
				...
				if (!this.currentGameType.isCreative())
				{
					...
					if (!itemstack1.isNotValid())
					{
						...
						if (itemstack1.isNotValid())
						{
							this.mc.player.setHeldItem(EnumHand.MAIN_HAND, ItemStack.EMPTY_ITEM_STACK);
						}
					}
				}
	...
	{
		ItemStack itemstack = this.mc.player.getHeldItemMainhand();
		boolean flag = this.currentItemHittingBlock.isNotValid() && itemstack.isNotValid();

		if (!this.currentItemHittingBlock.isNotValid() && !itemstack.isNotValid())
		{
	...
	public EnumActionResult processRightClickBlock(EntityPlayerSP player, WorldClient worldIn, BlockPos stack, EnumFacing pos, Vec3d facing, EnumHand vec)
	{
		...
		else
		{
			if (this.currentGameType != GameType.SPECTATOR)
			{
				...
				if ((!player.isSneaking() || player.getHeldItemMainhand().isNotValid() && player.getHeldItemOffhand().isNotValid()) && iblockstate.getBlock().onBlockActivated(worldIn, stack, iblockstate, player, vec, pos, f, f1, f2))
				{
					flag = true;
				}
	...
	/**
	 * Handles right clicking an entity from the entities side, sends a packet
	 * to the server.
	 */
	...
	{
		if (this.currentGameType.isCreative() && !itemStackIn.isNotValid())
		{
