...
public class EnchantmentHelper
{
	 ...
	 * 
	 * @param player
	 *            The player to check enchantment for
	 */
	...
	public static ItemStack getEnchantedItem(Enchantment p_92099_0_, EntityLivingBase p_92099_1_)
	{
		...
		else
		{
			...
			{
				if (!itemstack.isNotValid() && getEnchantmentLevel(p_92099_0_, itemstack) > 0)
				{
					list1.add(itemstack);
				}
			}

			return list1.isEmpty() ? ItemStack.EMPTY_ITEM_STACK : (ItemStack) list1.get(p_92099_1_.getRNG().nextInt(list1.size()));
		}
	}

	/**
	 * Returns the enchantability of itemstack, using a separate calculation for
	 * each enchantNum (0, 1 or 2), cutting to the max enchantability power of
	 * the table, which is locked to a max of 15.
	 */
