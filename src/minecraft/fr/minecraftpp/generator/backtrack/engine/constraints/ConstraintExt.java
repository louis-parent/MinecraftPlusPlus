 ...
 *
 */
public class ConstraintExt extends ConstraintPartielle
{

	private ArrayList<ArrayList<Object>> tuples; // ensemble des tuples de la
													// contrainte

	/**
	 * Construit une contrainte d'extension vide à partir d'une liste de
	 * variables
	 * 
	 * @param var
	 ...
	 * textuelle de la contrainte. La liste de variables est donnée sous la
	 * forme : Var1;Var2;... Puis le nombre de tuples est indiqué et enfin
	 * chaque tupe est donné sous la forme d'une suite de valeurs "String" :
	 * Val1;Val2;... Aucune vérification n'est prévue si la syntaxe n'est pas
	 * respectée !!
	 * 
	 * @param in
	...
	public ConstraintExt(BufferedReader in) throws Exception
	{
		super(in);
		tuples = new ArrayList<ArrayList<Object>>();
		int nbTuples = Integer.parseInt(in.readLine()); // nombre de tuples de
														// valeurs
	...
	/**
	 * Teste si une assignation viole la contrainte. La violation "classique"
	 * n'est avérée que si : - toutes les variables de la contrainte ont une
	 * valeur associée dans l'assignation testée - le tuple de valeur pour les
	 * variables de la contrainte dans l'assignation testée n'appartient pas
	 * aux tuples autorisés par la contrainte
	 * 
	 * @param a
	...
	public boolean violation(Assignment a)
	{
		...
		{
			boolean respectTuple = true;

			for (int j = 0; j < varList.size(); j++)
			{
				if (a.getVars().contains(varList.get(j)))
				{
					respectTuple &= (a.get(varList.get(j)).equals(tuple.get(j)));
				}
			}
